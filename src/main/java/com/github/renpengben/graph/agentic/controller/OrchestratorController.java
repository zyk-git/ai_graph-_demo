...
public class OrchestratorController {
    ...
    @Autowired
    public OrchestratorController(@Qualifier("orchestratorGraph") StateGraph orchestratorGraph)
            throws GraphStateException {
        SaverConfig saverConfig = SaverConfig.builder().build();
        // 编译时可设中断点
        this.compiledGraph = orchestratorGraph
                .compile(CompileConfig.builder().saverConfig(saverConfig).build());
    }
    @PostMapping
    public Map<String, Object> orchestrator(@RequestBody String text) {
        return compiledGraph.invoke(Map.of("inputText", text)).get().data();
    }

    @PostMapping(path = "/stream", produces = "text/event-stream")
    public Flux<Map<String, Object>> orchestratorStream(@RequestBody String text) {
