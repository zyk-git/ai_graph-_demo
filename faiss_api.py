...
class FaissSearchService:
    ...
    def build_rag_context(
        self, query: str, top_k: int, model: str | None
    ) -> tuple[list[SearchItem], str, str]:
        """检索并拼接上下文，产出可直接作为下游 LLM 输入的增强提示词。"""
        q = query.strip()
        if not q:
            raise ValueError("query 不能为空")

        hits = self.search(query=q, top_k=top_k, model=model)
        context_parts: list[str] = []
        for i, item in enumerate(hits, start=1):
            if not item.text:
                continue
            context_parts.append(f"[{i}] id={item.id} score={item.score:.6f}\n{item.text}")
        context = "\n\n".join(context_parts)

        if context:
            augmented_prompt = (
                "你是一个基于检索内容回答问题的助手。\n"
                "请优先依据下面的检索内容回答，若信息不足请明确说明。\n\n"
                f"检索内容:\n{context}\n\n"
                f"用户问题:\n{q}"
            )
        else:
            augmented_prompt = (
                "你是一个基于检索内容回答问题的助手。\n"
                "当前没有命中可用检索内容，请基于用户问题谨慎回答并说明不确定性。\n\n"
                f"用户问题:\n{q}"
            )

        return hits, context, augmented_prompt
...
def rag(req: SearchRequest) -> RagResponse:
    """完整链路：接收用户输入，检索并返回拼接后的增强提示词。"""
    try:
        s = _get_service()
        hits, context, augmented_prompt = s.build_rag_context(
            query=req.query, top_k=req.top_k, model=req.model
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
    except (FileNotFoundError, RuntimeError):
